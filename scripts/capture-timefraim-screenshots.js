 ...
 *   - The active timer is stopped through SQL between shots because the app's
 *     timer routes stamp the real clock, which would disagree with the fixed
 *     browser time.
 *
...
async function newContext(browser, colorScheme, storageState) {
...
(async () => {
  ...
  try {
    const { context, page } = await newContext(browser, 'dark');
    ...
    const light = await newContext(browser, 'light', storageState);
