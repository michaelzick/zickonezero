...
- **TimeFraim and 12 Step Meetings:** `/timefraim` and `/12-step-meetings` use the landscape showcase with four UX sections each. Their WebP images live in `public/img/projects/timefraim/` and `public/img/projects/12-step-meetings/`; TimeFraim captures come from the isolated local sandbox in `scripts/timefraim-sandbox/` (seeded sample planner data, never real data) and 12 Step Meetings captures come from the live site. Both projects have 512px icon squares and follow Riptyde and DemoStoke Fleet Ops in the homepage product engineering grid (`src/data/worksData.json` is reversed at load time, so the last entries show first). The shared `src/components/projectLinks.ts` list drives the desktop navigation, mobile navigation, and footer product engineering links, ordered Riptyde, DemoStoke Fleet Ops, TimeFraim, 12 Step Meetings, then the remaining projects.
