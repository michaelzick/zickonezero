...
- **TimeFraim and 12 Step Meetings:** `/timefraim` and `/12-step-meetings` use the landscape showcase with four UX sections each. Their WebP images live in `public/img/projects/timefraim/` and `public/img/projects/12-step-meetings/`; TimeFraim captures use the app's actual components with sample planner data. Both projects have 512px icon squares and lead the homepage UX Design grid. The shared `src/components/projectLinks.ts` list includes both projects first and drives the desktop navigation, mobile navigation, and footer UX Design links.
